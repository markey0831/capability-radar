...
## 十三、2026-09-08 正式域名上线与登录问题排查

### 线上信息（更新）

- 正式域名：`https://hfxwiu.top/`
- 首页：`https://hfxwiu.top/`
- 管理员后台：`https://hfxwiu.top/#/admin`
- 问卷（销售示例）：`https://hfxwiu.top/#/q/sales`
- 管理员密码：`bluefocus2026`（可在后台「修改密码」自助修改）
- CloudBase 环境：`bluefocus-d3grkg8s6e8a44be6`
- API 默认地址：`https://bluefocus-d3grkg8s6e8a44be6-1467982443.ap-shanghai.app.tcloudbase.com`

### 今天已完成的功能

1. 正式域名 `hfxwiu.top` 已绑定 CloudBase HTTP 访问服务，SSL 证书正常。
2. 首页改成入口引导页：管理员入口、评估人入口（7 个岗位下拉）、旧版单机能力图。
3. 前端路由改为 hash 路由，避免静态托管 SPA 刷新 404。
4. 管理员自助修改密码：密码哈希改存数据库 `admin_settings` 集合，新增 `POST /admin/change-password` 接口和后台「修改密码」页面。
5. UI 优化：按钮和文字居中。
6. 前端 API 地址改为相对路径 `/api`，兼容 http/https。
7. 修复无请求体写接口的 `Content-Type` 校验问题。

### 当前问题与已做排查

问题：管理员登录输入密码后，偶发（用户侧表现为持续）显示「网络连接失败，请稍后重试」。

根因判断：

- API 云函数冷启动约 6~11 秒，休眠唤醒期间连接会被重置。
- 登录成功后后台会并发加载 角色/人员/批次 三个接口，并发请求会触发多个新实例同时冷启动，加剧失败。

已做的修复（都已部署）：

- 云函数内存 256MB → 512MB。
- 前端请求超时 30 秒，失败自动重试（间隔 5 秒、10 秒）。
- `capability-radar-close-expired` 每 5 分钟顺带 `fetch` 一次 API health，保持 HTTP 侧实例温暖。
- 管理后台改为顺序加载（角色 → 人员 → 批次），避免并发冷启动。
- 前端 API 改为相对 `/api`。

### 明天继续

1. 让用户最终验证管理员登录是否成功（最新版已部署，需 Ctrl+F5 强制刷新）。
2. 若仍失败，继续排查用户浏览器/网络环境（已排除缓存、扩展、代理软件、HTTP/HTTPS、Edge 安全设置等因素）。
3. 本地 git 有 1 个未推送的 commit：`ee330d1`（管理后台数据改为顺序加载）。网络恢复后执行 `git push origin master`。
4. 推送后做一次完整端到端测试：管理员建人员 → 建批次 → 分配评分人 → 开放批次 → 问卷提交 → 结果导出。
